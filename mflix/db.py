...
def get_db():
    ...
    if db is None:
        ...
        # Set the maximum connection pool size to 50 active connections at the same time
        maxPoolSize=50,
        # TODO: Timeouts
        # Set the write timeout limit to 2500 milliseconds / Connection timeout is the number of milliseconds the driver will wait before a new connection attempt is aborted.
        wTimeoutMS=2500
        )[MFLIX_DB_NAME]
    return db


# Use LocalProxy to read the global db instance with just `db` / Allow us to not impliment connexion ID and Password on the function everytime
...
def get_movies_faceted(filters, page, movies_per_page):
    ...
    Faceted movies is a way to narrowing down searching in a collection.
    """
    # 
